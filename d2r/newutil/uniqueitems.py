...
def get(version):
    ...
    df['base_item_level'] = df.apply(
        calc_base_item_level, args=(weapons,), axis=1)
    df['base_item_level'] = df.apply(
        calc_base_item_level, args=(armor,), axis=1)
    df['base_item_level'] = df.apply(
        calc_base_item_level, args=(misc,), axis=1)

    assert df['base_item_level'].isnull().sum(
    ) == 0, f"One or more of the `base_item_levels` are null"

    # Lookup their base items types
    df['base_item_type'] = df.apply(
        calc_base_item_type, args=(weapons, "weap"), axis=1)
    df['base_item_type'] = df.apply(
        calc_base_item_type, args=(armor, "armo"), axis=1)
    df['base_item_type'] = df.apply(
        calc_base_item_type, args=(misc, 'misc'), axis=1)

    assert df['base_item_type'].isnull().sum(
    ) == 0, f"One or more of the `base_item_types` are null"
    ...
    # Save for debugging
    df.to_json(path.join(get_generated_dir(), "uniqueitems.json"),
