...
def get(version):
    ...
    df['TC'] = df.apply(get_treasure_class_for_difficulty, args=(treasureclassex, 'TreasureClass3',), axis=1)
    df['TC(N)'] = df.apply(get_treasure_class_for_difficulty, args=(treasureclassex, 'TreasureClass3(N)',), axis=1)
    df['TC(H)'] = df.apply(get_treasure_class_for_difficulty, args=(treasureclassex, 'TreasureClass3(H)',), axis=1)
    ...
    df['NameStr'] = df['Id'].apply(lambda Id: id_to_namestr_mapping[Id] if Id in id_to_namestr_mapping.keys() else Id)
    ...
    df.at[df['Id'] == 'diabloclone', 'Level'] = -9999
    df.at[df['Id'] == 'diabloclone', 'Level(N)'] = -9999

    # Save for debugging
    df.to_json(path.join(get_generated_dir(), "monstats.json"), orient='records', indent=2)
